#!/usr/bin/env python3
"""
Your original annotation script with smart resume feature
"""
...
class VenturaBeeAnnotator:
    def __init__(self, data_dir="data", start_frame=None):
        ...
        # Smart resume: find where you actually left off
        if start_frame is None:
            self.current_idx = self.find_resume_point()
        else:
            self.current_idx = start_frame - 1  # Convert to 0-indexed
        ...
        print(f"🚀 SMART RESUME: Starting from frame {self.current_idx + 1}")
        print(f"Images: {self.image_dir}")
        print(f"Labels: {self.label_dir}")
        
    def find_resume_point(self):
        """Find where you actually left off by checking recent file modifications"""
        import time
        
        # Look for files modified in the last 2 hours (your recent work)
        recent_cutoff = time.time() - (2 * 3600)
        recent_frames = []
        
        for i, img_file in enumerate(self.image_files):
            label_file = self.label_dir / (img_file.stem + ".txt")
            if label_file.exists() and label_file.stat().st_mtime > recent_cutoff:
                # Check if this has been manually refined (not default coordinates)
                with open(label_file, 'r') as f:
                    content = f.read().strip()
                    # Look for precise coordinates (manual work) vs default coordinates
                    if content and not any(coord in content for coord in ['0.250000', '0.500000', '0.750000']):
                        recent_frames.append(i)
        
        if recent_frames:
            # Start from the next frame after your last manual work
            last_manual_frame = max(recent_frames)
            resume_frame = min(last_manual_frame + 1, len(self.image_files) - 1)
            print(f"🔍 Detected last manual work at frame {last_manual_frame + 1}")
            print(f"📍 Resuming from frame {resume_frame + 1}")
            return resume_frame
        else:
            print("🔍 No recent manual work detected, starting from beginning")
            return 0
    ...
    def display_image(self):
        """Display image with current annotations - YOUR ORIGINAL VERSION"""
        if self.fig:
            plt.close(self.fig)
        ...
        # Add control buttons at bottom - EXACTLY YOUR ORIGINAL LAYOUT
        button_width = 0.08
        button_height = 0.04
        button_y = 0.01
        ...
        plt.show(block=True)  # THIS IS KEY - BLOCK=TRUE KEEPS WINDOW OPEN
    ...
    def auto_fit_boxes(self, event=None):
        ...
        # Reasonable bee size range
        min_size = min(img_w, img_h) * 0.02
        max_size = min(img_w, img_h) * 0.25
    ...
    def start_annotation(self):
        ...
        print(f"\n🎯 STARTING FROM FRAME {self.current_idx + 1}")
        print(f"🖱️  Click & drag to draw bounding boxes")
        print(f"🔘 Use buttons: Auto-fit | Undo | Clear All | Save | Next →")
...
def main():
    ...
    # Optional: manually specify starting frame if needed
    # annotator = VenturaBeeAnnotator(data_dir, start_frame=96)  # Start from frame 96
    
    # Or use smart auto-resume (recommended)
